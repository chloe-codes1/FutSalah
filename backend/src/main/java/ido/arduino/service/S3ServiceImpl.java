...
public class S3ServiceImpl implements S3Service {
	...
	public void uploadFile(final MultipartFile multipartFile, int ID, String status) {
		...
		try {
			...
			uploadFileToS3Bucket(bucketName, file, ID, status);
	...
	private void uploadFileToS3Bucket(final String bucketName, final File file, int ID, String status) {
		String uniqueFileName = LocalDateTime.now() + "_" + file.getName();
		LOGGER.info("Uploading file with name= " + uniqueFileName);
		if (status == "user") {
			userMapper.uploadProfileImage(ID, uniqueFileName);
		} else if (status == "team") {
			teamMapper.uploadProfileImage(ID, uniqueFileName);
		}
